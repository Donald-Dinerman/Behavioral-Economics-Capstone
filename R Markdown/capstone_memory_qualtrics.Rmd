...
mat_retention_fin_1 %>%
  ...
  coord_flip() +
  labs(title = "Average Incorrect Recollection Percentage", x = "Condition", y = "Recollection Percentage (%)", 
...
mat_retention_fin_1 %>%
  ...
  scale_x_discrete(limits = position[length(position):1]) +
  coord_flip() +
  labs(title = "Average Correct Recollection Percentage", x = "Condition", y = "Recollection Percentage (%)", 
       subtitle = "Error bars represent 95% confidence interval")
```
...
There may be a positive relationship between memory retention and renewal likelihoods that should be explored in the modeling section via regression analysis.
...
4. Participants spent less time on the screen in the reduced condition, but remembered the same number of benefits as the control and other treatment conditions, on average.
