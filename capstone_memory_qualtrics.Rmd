...
knitr::opts_chunk$set(echo = F, fig.align = 'center', out.width = '75%')
```
...
```{r, echo = F}
#show image
im = image_read("Images/Picture4.png")
image_trim(im)
```
...
```{r, echo = F}
im = image_read("Images/Picture1.png")
image_trim(im)
```
...
```{r, echo = F}
im = image_read("Images/Picture2.png")
image_trim(im)
```
...
```{r, echo = F}
im = image_read("Images/Picture3.png")
image_trim(im)
```
...
```{r, echo = F}
im = image_read("Images/Picture5.png")
image_trim(im)
```
...
```{r}
#load data
df_pre_dummy = read.csv("Data/Pretest_data.csv", skip = 1)

df_pre = read.csv("Data/Presurvey_nov3.csv", skip = 1)
...
Table 1: Top 5 Pre-Test Benefits

# Experiment

```{r}
#load data
df = read.csv("Data/Memory Project_November 11, 2021_16.19_buffer_count.csv", header = T)
#colnames(df)
#head(df)
```
...
Table 2: Sample Size Across Conditions
...
ggplot(mat_retention_fin_1) +
  geom_boxplot(aes(x = label, y = page_time), col = "black", fill = "steelblue", alpha = 0.5) +
  scale_x_discrete(limits = position) +
...
temp = summary(post_test)

new_lab = c('Categorization - Control = 0', 'Ranking - Control = 0', 'Reduced List - Control = 0')

data.frame(Estimate = temp$test$coefficients, 'Std.Error' = temp$test$sigma, 'P.Value' = temp$test$pvalues[1:3]) %>%  
  round(3) %>%
  tibble::rownames_to_column(., 'temp') %>%
  mutate('Linear Hypotheses' = new_lab, .before = Estimate) %>%
  dplyr::select(., -temp) %>%
  kbl(caption = "", booktabs = T) %>% 
  kable_paper("hover", full_width = T)
```
...
pw_temp = pairwise.wilcox.test(mat_retention_fin_1$Renewal.Liklihood_1, mat_retention_fin_1$label,
                 p.adjust.method = "BH") #no p-value correction, alt use "BH"

pw_temp$p.value %>% data.frame() %>% round(3) %>% mutate_all(as.character) %>% 
  tidyr::replace_na(., list(Control = "-", Categorization = "-", Ranking = "-")) %>%
  kbl(caption = "", booktabs = T) %>% 
  kable_paper("hover", full_width = T)
```
...
reg_name = c('(Intercept)', 'Memory Score', 'Age Bucket (35-50)', 'Age Bucket (>50)')

tab_model(reg_1, show.ci = F, show.stat = T, show.se = T, pred.labels = reg_name, dv.labels = 'Renewal Liklihood^2')
```
<br><br>
